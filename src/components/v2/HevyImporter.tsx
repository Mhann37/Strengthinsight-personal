...
// ── Muscle group inference (reused from Uploader) ─────────────
...
const HevyImporter: React.FC<HevyImporterProps> = ({ onWorkoutsExtracted, existingWorkouts }) => {
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          if (!sessionMap.has(key)) {
            sessionMap.set(key, { title, date: start, rows: [] });
          }
          sessionMap.get(key)!.rows.push(fields);
        }

        // Convert to SessionPreview[]
        ...
        sessionMap.forEach(({ title, date, rows }) => {
          ...
          for (const fields of rows) {
            ...
            if (!exerciseMap.has(exerciseName)) {
              exerciseMap.set(exerciseName, {
                sets: [],
                muscleGroup: inferMuscleGroupFromName(exerciseName),
              });
            }
          ...
          const isDup = existingWorkouts.some((ew) => {
            return ew.date && isoDate && ew.date.slice(0, 16) === isoDate.slice(0, 16);
          });
        ...
        if (previews.length === 0) {
          throw new Error('No workout sessions found in the CSV.');
        }
  ...
  const handleImport = async () => {
    ...
    // Import one at a time to handle duplicate detection gracefully
    for (const session of toImport) {
      try {
        await onWorkoutsExtracted([session.workout]);
        saved++;
        setSessions((prev) => {
          if (!prev) return prev;
        ...
        // Mark as duplicate if error indicates that
      ...
      {sessions && (
        <>
          <div className="flex items-center justify-between">
            ...
            {savedCount > 0 && (
              <p className="text-sm text-emerald-400 font-bold">{savedCount} imported</p>
            )}
          </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {session.status === 'duplicate' && (
                      <span className="text-[9px] font-black bg-orange-500/10 text-orange-500 px-1.5 py-0.5 rounded border border-orange-500/20 uppercase">Already exists</span>
                    )}
                    {session.status === 'saved' && (
                      <span className="text-[9px] font-black bg-emerald-500/10 text-emerald-500 px-1.5 py-0.5 rounded border border-emerald-500/20 uppercase">Imported</span>
                    )}
                  </div>
