...
export async function processWorkoutScreenshots(images: ImagePayload[]) {
  ...
  const payload = images.map((img) => {
    ...
    return {
      base64: parts.base64, // raw base64 only (smaller)
  ...
  try {
    // ✅ Critical: attach App Check token if enforcement is enabled
    ...
    // Our server sends JSON as the error message sometimes
    try {
